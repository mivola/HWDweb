...
//abfrage nach startzeit des mit einem joker gesetzen spiel
//echo "SELECT * FROM ".$season."_tbl_bet b, ".$season."_tbl_game g WHERE b.userID=".$act_userid." AND b.joker1=1 AND g.id=b.game AND g.play=".$play;
$resultJoker = mysql_query("SELECT * FROM ".$season."_tbl_bet b, ".$season."_tbl_game g WHERE b.userID=".$act_userid." AND b.joker1=1 AND g.id=b.game AND g.play=".$play);
$joker1Set = 0;
if (mysql_num_rows($resultJoker) == 1) {
...
<?PHP
  ...
  <tr valign=bottom>
    <td colspan=6 align=right><br><input type="submit" name="Submit" value="Tipps speichern"></td>
    <?PHP
      if ($joker1Set <> 1) {
        echo "<td align=center>kein Joker<br><input type=\"radio\" name=\"joker1\" value=\"0\"></td>";
      }
    ?>
