...
$wins         = mysqli_query($connectedDb, "SELECT u.id, u.nick_name AS nick_name, SUM(w.wins) AS wins FROM tbl_user u, tbl_wins w WHERE u.id=w.userID GROUP BY u.id ORDER BY wins DESC");
$extra_wins   = mysqli_query($connectedDb, "SELECT u.id, u.nick_name AS nick_name, SUM(e.wins) AS wins FROM tbl_user u, tbl_extra_wins e WHERE u.id=e.userID GROUP BY u.id ORDER BY wins DESC");
$max_points   = mysqli_query($connectedDb, "SELECT u.id, u.nick_name, p.play, p.points FROM tbl_points p, tbl_user u WHERE u.id=p.userID ORDER BY points DESC, u.id, p.play");
$min_points   = mysqli_query($connectedDb, "SELECT u.id, u.nick_name, p.play, p.points FROM tbl_points p, tbl_user u WHERE u.id=p.userID ORDER BY points ASC, u.id, p.play");
$three_points = mysqli_query($connectedDb, "SELECT u.id, count(*) FROM tbl_game g, tbl_bet b, tbl_user u, tbl_play p WHERE p.id=g.play AND p.completed > 0 AND u.id=b.userid AND b.game=g.id AND b.bet1=g.result1 AND b.bet2=g.result2 GROUP BY u.id ORDER BY u.id");

$max_points_row = 5;


while ($row4 = mysqli_fetch_row($three_points)) {
...
while ($row = mysqli_fetch_row($wins)) {
  ...
  $points = mysqli_query($connectedDb, "SELECT u.id, u.nick_name AS nick_name, SUM(p.points) AS points, MAX(p.points) AS max, MIN(p.points) AS min, round(AVG(p.points), 2) AS avg FROM tbl_user u, tbl_points p WHERE u.id=p.userID AND u.id=".$id." GROUP BY u.id");
  while ($row2 = mysqli_fetch_row($points)) {
  ...
  $extra_wins = mysqli_query($connectedDb, "SELECT e.userid, e.wins FROM tbl_extra_wins e WHERE e.userid=".$id);
  while ($row3 = mysqli_fetch_row($extra_wins)) {
...
<?PHP
  ...
  $old_points = 0;
  for ($i = 1; $i <= mysqli_num_rows($max_points); $i++){
  
    $row = mysqli_fetch_array($max_points);
...
<?PHP
  ...
  $old_points = 0;
  for ($i = 1; $i <= mysqli_num_rows($min_points); $i++){
  
    $row = mysqli_fetch_array($min_points);
