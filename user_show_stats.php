...
while ($row = mysql_fetch_row ($wins)) {
  ...
  $points = mysql_query("SELECT u.id, u.nick_name AS nick_name, SUM(p.points) AS points, MAX(p.points) AS max, MIN(p.points) AS min, round(AVG(p.points), 2) AS avg FROM tbl_user u, tbl_points p WHERE u.id=p.userID AND u.id=".$id." GROUP BY u.id");
