...
$play_str = "SELECT g.play AS play, max(g.p_ts) AS p_ts FROM ".$season."_tbl_play p, ".$season."_tbl_game g WHERE p.id=g.play AND p.recorded > 0 AND completed = 0 AND season=".$season." AND g.play=3 GROUP BY g.play";
$play = mysql_query($play_str);
$resultPlays = mysql_query("SELECT * FROM ".$season."_tbl_play WHERE completed>0 AND id>3");
