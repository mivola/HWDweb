...
$result = mysql_query("SELECT * FROM ".$season."_tbl_extra_wins e WHERE userID=0");
...
// nur bis zum 3. Spieltag
$play_str = "SELECT g.play AS play, max(g.p_ts) AS p_ts FROM ".$season."_tbl_play p, ".$season."_tbl_game g WHERE p.id=g.play AND p.recorded > 0 AND completed = 0 AND season=".$season." GROUP BY g.play";
$play_str = "SELECT g.play AS play, min(g.p_ts) AS p_ts FROM ".$season."_tbl_play p, ".$season."_tbl_game g WHERE p.id=g.play AND p.recorded > 0 AND completed = 0 AND season=".$season." AND g.play=1 GROUP BY g.play";
//echo $play_str;
$play = mysql_query($play_str);
$resultPlays = mysql_query("SELECT * FROM ".$season."_tbl_play WHERE completed>0 AND id>1");
//$str = "SELECT g.play AS play, max(g.p_ts) AS p_ts FROM ".$season."_tbl_play p, ".$season."_tbl_game g WHERE p.id=g.play AND season=".$season." AND p.id=3 AND p_ts<SYSDATE() GROUP BY g.play";
//$resultPlays = mysql_query($str);

$result = mysql_query("SELECT e.* FROM ".$season."_tbl_user u, ".$season."_tbl_extra_wins e WHERE u.id=".$act_userid." AND e.userID=u.id");
...
//$result_users = mysql_query("SELECT u.nick_name, e.* FROM ".$season."_tbl_user u, ".$season."_tbl_extra_wins e WHERE u.id=e.userID");

$resultUsers = mysql_query("SELECT * FROM ".$season."_tbl_user u, ".$season."_tbl_extra_wins e WHERE u.id=e.userID ORDER BY u.id");
//echo "SELECT * FROM ".$season."_tbl_user u, ".$season."_tbl_extra_wins e WHERE u.id=e.userID ORDER BY u.id";
...
//1.Absteiger
    case 5:
      echo "<td align=right>Absteiger (Platz 16)</td>";
    ...
    //3.Aufsteiger
    case 10:
      echo "<td align=right>Aufsteiger (Platz 3)</td>";
      echo "<td align=center align=center>";
      if ($act_up3 == 0) { echo"---"; }
      else {
        for ($j=0; $j<count($BL2); $j++){
