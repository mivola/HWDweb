...
$result1 = mysql_query("DELETE FROM ".$season."_tbl_game WHERE play=".$play);
if (isset($rueck)) {
  $p = $playID + 17;
  $result1 = mysql_query("DELETE FROM ".$season."_tbl_game WHERE play=".$p);
}
...
//mktime(8,30,0,5,29,2001)."
  ...
  $query = "INSERT INTO ".$season."_tbl_game (play, team1, team2, p_ts) VALUES (".$playID.", ".$team1.", ".$team2.", ".$mktime.")";
  $resultBL1[$i] = mysql_query($query);

  if (isset($rueck) && $i < 10) {
    $p = $playID + 17;
    $query = "INSERT INTO ".$season."_tbl_game (play, team1, team2) VALUES (".$p.", ".$team2.", ".$team1.")";
...
if (isset($free)) {
  if ($maxi==9) {
    $result1 = mysql_query("UPDATE ".$season."_tbl_play SET recorded=1 WHERE id=".$play);
  }
  else {
    $result1 = mysql_query("UPDATE ".$season."_tbl_play SET recorded=2 WHERE id=".$play);
  }
...
// Select * from ".$season."_tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=2 and t1.id=g.team1 and t2.id2=g.team2 and t1.league=1
...
// select * from ".$season."_tbl_game g, ".$season."_tbl_team1 t1 where g.play=1 and t1.id=g.team1
// select * from ".$season."_tbl_game AS g, ".$season."_tbl_team1 AS t1, ".$season."_tbl_team1 AS t2 where g.play=1 and t1.id=g.team1 and t2.id=g.team2
