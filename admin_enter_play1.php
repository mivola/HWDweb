...
$resultPlayBL1 = mysql_query("Select t1.id, t2.id2, g.p_ts from ".$season."_tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=1 order by g.id");
$stringResultPlayBL1 = "Select t1.id, t2.id2, g.p_ts from ".$season."_tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=1 order by g.id";
//echo "Select t1.id, t2.id2, g.p_ts from ".$season."_tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=1 order by g.id";
...
$games2=1;
//$resultPlayBL2 = mysql_query("Select * from ".$season."_tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=2 order by g.id");
$resultPlayBL2 = mysql_query("Select t1.id, t2.id2, g.p_ts from ".$season."_tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=2 order by g.p_ts, g.id");
$stringResultPlayBL2 = "Select t1.id, t2.id2, g.p_ts from ".$season."_tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=2 order by g.id";
if (mysql_num_rows($resultPlayBL2) == 0) {
  $games2=0;
//  $resultPlayBL2 = mysql_query("Select * from ".$season."_tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=2 order by g.id");
}

$str = "Select * from ".$season."_tbl_team1 where league=1 order by name";
...
$str = "Select * from ".$season."_tbl_team1 where league=2 order by name";
...
<input name=playID type=hidden value=<?PHP echo $play ?>>
  <table border="0">
    <tr><td colspan=4><b>1. Bundesliga</b><br>
    <font size=3><a href="http://www.dfb.de/bliga/spiel.php?liga=bl1m&spieltag=<?PHP echo $play ?>&art=e&saison=0<?PHP echo $season ?>&saisonl=200<?PHP echo $season ?>&lang=D" target="_blank">zur DFB-Seite</a></font>
    </td></tr>
...
<?PHP
  ...
  for ($j=1; $j<=9; $j++){
    ...
    // mit JavaScript für die nachfolgenden übernehmen
    if ($j == 1) {
      //echo "<td><a href=#>für alle</a></td>";
...
?>
    ...
    <font size=3><a href="http://www.dfb.de/bliga/spiel.php?liga=bl2m&spieltag=<?PHP echo $play ?>&art=e&saison=0<?PHP echo $season ?>&saisonl=200<?PHP echo $season ?>&lang=D" target="_blank">zur DFB-Seite</a>
    </td></tr>    
    <tr style="background-color:<?PHP echo $table_head; ?>;">
      <td valign=top><b>SpNr.</b></td>
      <td valign=top><b>&uuml;ber-<br>nehmen</b></td>
