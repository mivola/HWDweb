...
for ($i=1; $i <= $maxi; $i++){
  ...
  if ($bet1 == "") { $bet1 = "-1"; }
  if ($bet2 == "") { $bet2 = "-1"; }
  $result1 = mysql_query("DELETE FROM tbl_bet WHERE game=".$game." AND userID=".$new_user_id);
  $query = "INSERT INTO tbl_bet (userID, game, bet1, bet2, bet_ts) VALUES (".$new_user_id.", ".$game.", ".$bet1.", ".$bet2.", ".time().")";
  //echo $query;
...
//Select * from tbl_bet b, tbl_game g, tbl_team1 t1, tbl_team2 t2 where play=1 and t1.id=g.team1 and t2.id2=g.team2 and t1.league=1 and b.game=g.id
//$resultPlayBL1 = mysql_query("Select * from tbl_game g, tbl_team1 t1, tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=1");
$resultPlayBL1 = mysql_query("Select * from tbl_bet b, tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=1 and b.game=g.id and b.userID=".$new_user_id." order by g.id");
$resultPlayBL2 = mysql_query("Select * from tbl_bet b, tbl_game g, ".$season."_tbl_team1 t1, ".$season."_tbl_team2 t2 where play=".$play." and t1.id=g.team1 and t2.id2=g.team2 and t1.league=2 and b.game=g.id and b.userID=".$new_user_id." order by g.id");
$username = mysql_fetch_row(mysql_query("SELECT nick_name from tbl_user WHERE id=".$new_user_id));

//$resultBL1 = mysql_query("Select * from tbl_team1 where league=1 order by name");
//$resultBL2 = mysql_query("Select * from tbl_team1 where league=2 order by name");
